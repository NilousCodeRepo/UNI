...
def func1(string_list, word):
    ...
    for el in string_list:
        if fl in el:
            counter += 1
    string_list[:] = [ w for w in string_list if fl not in w ]
#questo è corretto perchè string_list[:] modfica l'oggetto, mentre string_list ne crea uno nuovo
    
    return counter

string_list = ['cocacola','fanta', 'assenzio', 'Minosse', 'sinfonie']
word = 'saponette'
#print(func1(string_list, word))
...
def func2(pathname):
    D = {}
    if_value_not_exist = []
    with open(pathname, "r") as fi:
        for line in fi:
            N,M = line.strip().split(",")
            N = int(N)
            D[M] = D.get(M,if_value_not_exist) + [N]#+ [N] concatenates
...
fp = "func2/func2_test_1.txt"
#print(func2(fp))


# %% ----------------------------------- FUNC3 ------------------------- #
'''  func3: 2 points
Implementare la funzione func3(listA, pathname) che prende in input una lista di stringhe listA e una stringa che indica il percorso di un file pathname. 
...
def func3(listaA, pathname):
    listaA = sorted(listaA, key = lambda x: (len(x), x[-1].lower()))
    counter = 0

    with open(pathname, "w") as of:
        for el in listaA:
            print(el, file=of)

    for el in range(len(listaA)):
        for i in listaA[el]:
            counter += 1
    return counter
...
def func4(S):
    ...
    for el in sorted(D, reverse = True):
        ...
        istogramma += pad + word + asterisk + newline 
...
def func5(img, output_file_name):
    ...
    img2 = [ [ img[Y(x)][X(y,W)] for x in range(H) ] for y in range(W) ]
...
1) Prende in input una stringa che indica il percorso di una directory ed esplora ricorsivamente l'albero delle directory restituendo un dizionario. 

La CHIAVE del dizionario è il percorso assoluto a partire dalla directory `dizionario` in forma di stringa. 
Il VALORE corrisponde a una stringa costruita come segue:

1b) Prendiamo tutti i file della directory dove siamo solo con estensione `.txt` ordinati in ordine alfabetico inverso. 

I file `.txt` sono file di testo in cui ogni riga contiene una serie di numeri interi separati da spazi. 
...
leggere sequenzialmente ogni numero convertendono in un carattere. La concatenazione di tutti i caratteri forma una stringa.

Ad esempio, la sequenza sopra viene convertita nella stringa `KTTISLJL`.

Il valore nel dizionario è la stringa ottenuta concatenando le stringhe generate per ciascun file di testo di quella directory secondo l'ordine alfabetico inverso dei nomi dei file `.txt`.
...
def get_text(path):
    ...
    for file in os.listdir(path):
        ...
        if os.path.isdir(fullpath):
            D.update(get_text(fullpath))

        elif file.endswith('.txt'):
            D[path].append(fullpath)
...
altre funzioni ricorsive, che:

1) Prende in ingresso una lista di numeri interi positivi 'nums' e una lista di stringhe 'ops' che indicano delle operazioni binarie tra numeri. 

NOTA: le due liste possono contenere ripetizioni.

La funzione deve generare ricorsivamente tutte le possibili espressioni aritmetiche, dove ciascuna espressione e' una stringa. 

Le espressioni derivano dalla concatenzione di numeri presi da 'nums' intervallati da operazioni prese da 'ops'. 
...
~~~ puoi usare solo n num e m sym : n e m siamo il numero di volte che n e m si ripeto nelle loro liste ~~~
...
NOTA: NON  definite la funzione ricorsiva internamente a ex2() altrimenti non passate il test ricorsivo.
