...
def func1 (dict1, dict2):
    ...
    with open(dict1) as F1:
        diz1 = dict( [ coppia.split()                       # le coppie chiave valore di una riga
                            for coppia in F1                # per ogni riga
                            if len(coppia.split()) == 2     # solo se sono 2
                    ] )
    with open(dict2) as F2:
        diz2 = dict( [ coppia.split() for coppia in F2 if len(coppia.split()) == 2 ] )
    return { k: "-".join(sorted([ diz1[k],diz2[k]] ))           # valori in ordine alfabetico, uniti da '-'
                for k in set(diz1.keys()) & set(diz2.keys()) }  # per tutte le chiavi comuni
