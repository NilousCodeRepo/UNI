...
Si progetti una funzione che:
 
 - Riceve come argomenti i path di due file dict1 e dict2 e 
	 ritorni come output un dizionario.

I due file contengono delle righe di testo con parole separate da spazi e tab,
...
"""
def func1 (path1:str, path2:str) -> dict:
	with open(path1) as F1:
		dict1 = dict( [ riga.split() for riga in F1 if len(riga.split()) == 2 ] )
	
	with open(path2) as F2:
		dict2 = dict( [ riga.split() for riga in F2 if len(riga.split()) == 2 ] )

	final_dict = { k: "-".join( sorted( [ dict1[k],dict2[k] ] ) )
								# valori in ordine alfabetico, uniti da '-'
                
								for k in set(dict1.keys()) & set(dict2.keys()) 
								# per tutte le chiavi comuni
							 }  
...
#%% ----------------------------------- FUNC4 ------------------------- #
""" func2: 6 punti
Si scriva una funzione func2(input_file, output_file) che:

- riceve come argomenti 'input_file' e 'output_file'. 
...
Definisci una funzione func3(file_in, file_out, length, chars) che 
...
La funzione deve restituire:
	...
	- L'elenco di tutte le parole trovate nel file puntato da file_in 
		con una lunghezza *almeno* length e contenente *almeno* uno dei
...
def func3(file_in : str, file_out: str, length:int, chars:str) -> list[str]:
	...
	with open(file_in) as FI:
		word_list = FI.read().split()
	
		word_list = [ word 
								  for word in word_list 
										if len(word) >= length and any(char in word for char in chars) 
								]
	word_list.sort(key = lambda x: (-len(x),x) )

	with open(file_out, mode = "w") as FO:
...
Il file indicato da textfile_in contiene dei numeri separati da spazi.

La funzione deve:
	- Leggere i numeri
		- Ordinarli in ordine decrescente di solo caratteri numerici 
		-	In caso di parità in ordine crescente di valore.

	- Scrivere questi numeri ordinati nel file textfile_out, separati da virgola e spazio. 
		
	- Restituire la quantità di numeri letti da textfile_in.
...
def func4(textfile_in, textfile_out):
	...
	with open(textfile_in) as TF_I:
		num_list = TF_I.read().split()
		
	num_list.sort(key=criterio)
	
	with open(textfile_out, 'w') as TF_O:
		TF_O.write(', '.join(num_list))
...
'''
Func 5: 6 punti
Si definisca la funzione func5(filein) che:
...
La matrice da restituire e' rappresentata come lista di liste.
...
def func5(input_filename):
	with open(input_filename) as IF:
		M = [ list(map(int, riga.split())) for riga in IF ]
	
	W = len(M[0])
	H = len(M)
    
	return [ [ M[H-1-y][W-1-x] for y in range(H) ]
         	for x in range(W)
				 ]
